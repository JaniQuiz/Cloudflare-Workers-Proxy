...
**在原仓库基础上改进内容：**
 ...
 - 具备推断无头 API 请求和管理 Cookie 穿透的能力，可以大幅度提高对复杂动态站点的代理兼容性。
 - **AXIOS / Fetch 拦截** 无论前端使用的是原生 fetch 还是封装好的 axios 库，它们底层发送请求前，都会被拦截器捕获，将path修改为对应的反向代理地址，从而实现解决网站登录等页面跳转不走反代问题。
 - 解决一些网站的SPA（如 Vue, React）在切换页面时重写 URL导致域名后继的刷新全部 404 问题。
 - 对WSS也会进行拦截并进行反向代理。
